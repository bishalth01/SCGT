...
def MSE(scores, targets):
    squared_errors = F.mse_loss(scores, targets)
    mse = squared_errors.detach().item()
    return mse


def CORR(scores, targets):
    # scores = scores.detach().cpu().numpy()
    # targets = targets.detach().cpu().numpy()
    # scores = scores.detach().cpu().numpy().flatten()  # Flatten the arrays
    # targets = targets.detach().cpu().numpy().flatten()
    
    corr, _ = pearsonr(scores, targets)
