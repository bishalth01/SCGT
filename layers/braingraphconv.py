...
class MyNNConv(NodeCommunityGraphTransformerLayer):
    def __init__(self, in_channels, out_channels, nn, normalize=False, bias=False,
                 **kwargs):
...
#        uniform(self.in_channels, self.weight)
        self.uniform(self.in_channels, self.bias)
    ...
    # def update(self, aggr_out):
    #     # SERO attention readout
    #     alpha = torch.tanh(aggr_out)
    #     alpha = F.softmax(alpha, dim=1)
    #     readout = (alpha * aggr_out).sum(dim=1)

    #     # learnable weights
    #     if self.bias is not None:
    #         readout = readout + self.bias
    #     if self.normalize:
